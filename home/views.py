...
def frequency(customerID, period):
    if ait(customerID, period) == customerID + ' 在這段時間沒購買任何東西喔!':
        ...
        if days == 0:
            days += int(period.rstrip('days'))

        if period == 'sofar':
            freq = int((datetime.now() - datetime(2021, 1, 1)).days / int(days))
            
        elif period == '365days':
            freq = int(1 * 365 / int(days))
        
        elif period == '180days':
            freq = int(1 * 180 / int(days))
            
        elif period == '90days':
            freq = int(1 * 90 / int(days))
            
        elif period == '30days':
            freq = int(1 * 30 / int(days))
